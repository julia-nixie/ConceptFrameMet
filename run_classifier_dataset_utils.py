...
def convert_two_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # set the target word as 1 in segment ids
        try:
            if isinstance(text_b, list) and len(text_b) > 0:
                tokens_b += 1  # add 1 to the target word index considering [CLS]
                for i in range(len(text_b)):
                    segment_ids[tokens_b + i] = 1
            ...
            if ex_index < 5:
                logger.warning(f"Failed to set segment_ids for example {ex_index}: {e}, text_b type={type(text_b)}, text_b={text_b}, tokens_b={tokens_b}")
...
def convert_examples_to_two_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # Set target word positions in segment_ids
        try:
            if isinstance(text_b, list) and len(text_b) > 0:
                tokens_b += 1  # add 1 to the target word index considering [CLS]
                for i in range(len(text_b)):
        ...
        try:
            if isinstance(text_b, list) and len(text_b) > 0:
                for i in range(len(text_b)):
                    if tokens_b + i < len(tokens_with_mask):
                        tokens_with_mask[tokens_b + i] = tokenizer.mask_token
        except (TypeError, IndexError) as e:
            if ex_index < 5:
                logger.warning(f"Failed to create masked tokens for example {ex_index}: {e}")
        ...
        # Second features (Target word + sentence) for QA-style models
        # Format: [CLS] target_word [SEP] full_sentence [SEP]
        tokens = [tokenizer.cls_token] + text_b + [tokenizer.sep_token] + tokens_a + [tokenizer.sep_token]
        segment_ids_2 = [0] * len(tokens)
        try:
            tokens_b = 1  # add 1 to the target word index considering [CLS]
            # Mark only the target word tokens (before first SEP) with 1
            for i in range(len(text_b)):
        ...
        # DEBUG: Print first few features to verify segment_ids_2
        if ex_index < 3:
            logger.info("*** input_ids_2 Debug ***")
            logger.info(f"tokens (input_ids_2): {' '.join([str(x) for x in tokens[:20]])}")
            logger.info(f"segment_ids_2 (first 20): {' '.join([str(x) for x in segment_ids_2[:20]])}")
            logger.info(f"Number of 1s in segment_ids_2: {sum(segment_ids_2)}")
            logger.info(f"Positions marked as 1: {[i for i, x in enumerate(segment_ids_2) if x == 1]}")
...
def all_metrics(preds, labels):
    ...
    # Binary metrics (positive class only)
    f1 = f1_score(y_true=labels, y_pred=preds, average='binary')
    pre = precision_score(y_true=labels, y_pred=preds, average='binary')
    rec = recall_score(y_true=labels, y_pred=preds, average='binary')
    
    # Micro-averaged metrics (global)
    f1_micro = f1_score(y_true=labels, y_pred=preds, average='micro')
    pre_micro = precision_score(y_true=labels, y_pred=preds, average='micro')
    rec_micro = recall_score(y_true=labels, y_pred=preds, average='micro')
    
    # Macro-averaged metrics (unweighted mean of both classes)
    f1_macro = f1_score(y_true=labels, y_pred=preds, average='macro')
    pre_macro = precision_score(y_true=labels, y_pred=preds, average='macro')
